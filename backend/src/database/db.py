...
# def store_users_foodPlanning_info(cursor, conn, user_id: str, survey_data: dict):
#     try:
#         cursor.execute("""
...
# def get_user_food_planning_info(cursor, user_id: str):
#     cursor.execute("SELECT * FROM food_planning WHERE user_id = %s", (user_id,))
#     return cursor.fetchall()


# # ✅ Get groceries by user
# def get_groceries_by_user(cursor, user_id: str):
#     cursor.execute("SELECT * FROM available_groceries WHERE user_id = %s", (user_id,))
#     return cursor.fetchall()


# # ✅ Insert a new grocery
# def add_grocery(cursor, conn, grocery_data: dict):
#     try:
#         cursor.execute("""
...
# # ✅ Update grocery
# def update_grocery(cursor, conn, grocery_id: int, updates: dict):
#     fields = ", ".join([f"{k}=%s" for k in updates.keys()])
#     values = list(updates.values()) + [grocery_id]
#     try:
#         cursor.execute(f"UPDATE available_groceries SET {fields} WHERE ID=%s", tuple(values))
#         conn.commit()
...
# # ✅ Delete grocery
# def delete_grocery(cursor, conn, grocery_id: int):
#     try:
#         cursor.execute("DELETE FROM available_groceries WHERE ID=%s", (grocery_id,))
#         conn.commit()
...
# # ✅ Add meal plan
# def add_meal_plan(cursor, conn, meal_data: dict):
#     try:
#         cursor.execute("""
...
#             json.dumps(meal_data["nutrition"]),      # convert dict → JSON
#             json.dumps(meal_data["recipe"]),         # steps list → JSON
#             json.dumps(meal_data["ingredients_used"]) # list of Ingredient objects → JSON
#         ))
#         conn.commit()
#         return {"id": cursor.lastrowid}
#     except Exception as e:
...
# def change_meal(cursor, conn, user_id: str, meal_data: dict):
#     try:
#         cursor.execute("""
#             UPDATE meal_plan 
#             SET meal_name=%s, 
...
#         """, (
#             meal_data['name'],
#             json.dumps(meal_data["nutrition"]),        # dict → JSON string
#             json.dumps(meal_data["recipe"]),          # list → JSON string
#             json.dumps(meal_data["ingredients_used"]),# list[dict] → JSON string
#             user_id, 
#             meal_data["day"], 
#             meal_data["meal_type"]
#         ))
...
# def delete_all_meal(cursor, conn, user_id: str):
#     try:
...
# def get_meal_plan(cursor, user_id: str):
#     cursor.execute("SELECT * FROM meal_plan WHERE user_id = %s", (user_id,))
#     return cursor.fetchall()
